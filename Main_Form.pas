...
uses
  ...
  StdCtrls, Grids, ExtCtrls, Buttons, ShellApi, Variants, ScktComp, Menus, ComCtrls, WinSock, about_form;
...
begin
  ...
  begin
    ...
    begin
      ...
      cygpath := StringReplace(ExtractFilePath(Filename), '\', '/', [rfReplaceAll, rfIgnoreCase]);//replace \s with /s
      Delete(cygpath, 2, 1); //delete colon

      if FlexcomAnl then
        ParamString := '/e:on /C "c:\progra~1\mcs\setpath794.bat" && Title ' + Project + ' && "' + Filename + '" | tee "/cygdrive/' + cygpath + Project + '.txt"' //title, filepath, filename, echofile
      else
        ParamString := '/C Title ' + Project + ' && "' + Filename + '" | tee "/cygdrive/' + cygpath + Project + '.txt"'; //title, filepath, filename, echofile
    ...
    TH_FOUND:
      begin
        ...
        if Listen.Checked and (pos('(From ', FoundRecord^.Project) <> 0) then
          SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + LocalHost + ': Project:' + FoundRecord^.Project + ', File:' + FoundRecord^.Filename + ' has successfully run.');
    ...
    TH_ERROR:
      begin
        ...
        outlist_sg.Cells[3, outlist_sg.RowCount - 1] := 'Error';
         ...
         //if job came from over network
        if Listen.Checked and (pos('(From ', FoundRecord^.Project) <> 0) then
          SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + LocalHost + ': Project:' + FoundRecord^.Project + ', File:' + FoundRecord^.Filename + ' has run into an error, please check the echo file.');
...
begin
  ...
  if not FileExists(DEFAULT_TEEFILE_LOC + 'tee.exe') then
  begin
    ResStream := TResourceStream.Create(
      HInstance, // your app or DLL instance handle
      'tee', // string containing resource name
      RT_RCDATA); // identifies RCDATA resource type
    ResStream.SaveToFile(DEFAULT_TEEFILE_LOC + 'tee.exe');
  end;

  if not FileExists(DEFAULT_TEEFILE_LOC + 'cygwin1.dll') then
  begin
    ResStream := TResourceStream.Create(
      HInstance, // your app or DLL instance handle
      'dll3', // string containing resource name
      RT_RCDATA); // identifies RCDATA resource type
    ResStream.SaveToFile(DEFAULT_TEEFILE_LOC + 'cygwin1.dll');
  end;

  if not FileExists(DEFAULT_TEEFILE_LOC + 'cygintl-8.dll') then
  begin
    ResStream := TResourceStream.Create(
      HInstance, // your app or DLL instance handle
      'dll2', // string containing resource name
      RT_RCDATA); // identifies RCDATA resource type
    ResStream.SaveToFile(DEFAULT_TEEFILE_LOC + 'cygintl-8.dll');
  end;

  if not FileExists(DEFAULT_TEEFILE_LOC + 'cygiconv-2.dll') then
  begin
    ResStream := TResourceStream.Create(
      HInstance, // your app or DLL instance handle
      'dll1', // string containing resource name
      RT_RCDATA); // identifies RCDATA resource type
    ResStream.SaveToFile(DEFAULT_TEEFILE_LOC + 'cygiconv-2.dll');
  end;

  MsgRes := MessageDlg('Are you using Program Queuer for executing Flexcom analyses?' + chr(vk_return) + chr(vk_return) + '(Selecting ''No'' will result in running windows terminals for analyses)', mtConfirmation, [mbYes, mbNo], 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      //check that file is not over the network
        if pos('\\', OpenDialog.Files[Counter]) <> 0 then
        begin
          MessageDlg('A file cannot be added over the network', mtError, [mbOk], 0);
          exit;
        end;
...
begin
  if (outlist_sg.Cells[3, 1] <> '') and ((outlist_sg.Col = 1) or (outlist_sg.Col = 2)) then
  begin
    ShellExecute(Application.Handle, 'open', 'explorer.exe',
...
begin
  ...
  begin
    StrPort := '80';
    ClickedOK := InputQuery('What port?', 'Please enter the port you want to listen. Make sure it''s a valid integer number between 1 and 65535.', StrPort);
    if (StrPort = '') or (length(StrPort) > 6) or not ClickedOK then
    begin
...
begin
  Button2Enabled := Button2.Enabled;
  Button3Enabled := Button3.Enabled;
  ...
  begin
    ...
    //check filepath is local
    if pos(Uppercase(Socket.LocalHost), Uppercase(filename)) = 0 then
    begin
      SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Error! Project:' + project + ', File:' + filename + ' filepath is not on the local machine.');
      Button2.Enabled := Button2Enabled;
      Button3.Enabled := Button3Enabled;
      exit;
    end;

    filepos := posex('\', filename, 3);
    if filepos <> 0 then
    begin
      filename := copy(filename, filepos + 1, length(filename) - filepos + 1);
      insert(':', filename, pos('\', filename));
    end
    else
    begin
      SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Error! Project:' + project + ', File:' + filename + ' filename is in the wrong format');
      Button2.Enabled := Button2Enabled;
      Button3.Enabled := Button3Enabled;
      exit;
    end;
    ...
    begin
      SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Error! Project:' + project + ', File:' + filename + ' the file does not exist');
      Button2.Enabled := Button2Enabled;
      Button3.Enabled := Button3Enabled;
      exit;
    end;
    ...
    if startclean and autostart then
      SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Project:' + project + ', File:' + filename + ' successfully running.')
    else
      SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Project:' + project + ', File:' + filename + ' successfully queued.');

  end
  else
  begin
    SendToAllClients('[' + Datetostr(Date) + ' ' + Timetostr(GetTime) + '] ' + Socket.LocalHost + ': Error! Project:' + project + ', File:' + filename + ' failed to be queued.');
    Button2.Enabled := Button2Enabled;
    Button3.Enabled := Button3Enabled;
  end;
end;
...
begin
  about_form.about_window.ShowModal;
end;
