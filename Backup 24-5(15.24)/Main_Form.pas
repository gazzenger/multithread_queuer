...
begin
  ...
  begin
    ...
    begin
      ...
      if FlexcomAnl then
        ParamString := '/e:on /C "c:\progra~1\mcs\setpath794.bat" & Title ' + Project + ' & cd "' + ExtractFilePath(Filename) + '" & "' + ExtractFileName(Filename) + '" | tee "' + Project + '.txt"' //title, filepath, filename, echofile
      else
...
begin
  ...
  MsgRes := MessageDlg('Are you using Program Queuer for executing Flexcom analyses?' + chr(vk_return) + chr(vk_return) + '(Selecting ''No'' will result in default to windows terminal)', mtConfirmation, [mbYes, mbNo], 0);
